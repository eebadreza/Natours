(() => {
    ...
    function isVisitable(thing) {
        return (
            utils_default.isPlainObject(thing) || utils_default.isArray(thing)
        );
    }
    ...
    function formDataToJSON(formData) {
        function buildPath(path, value, target, index) {
            ...
            name =
                !name && utils_default.isArray(target) ? target.length : name;
            if (isLast) {
            ...
            function transformResponse(data) {
                ...
                if (
                    ...
                    const strictJSONParsing =
                        !silentJSONParsing && JSONRequested;
                    try {
    ...
    utils_default.reduceDescriptors(
    ...
    var resolveConfig_default = (config) => {
        ...
        if (platform_default.hasStandardBrowserEnv) {
            ...
            if (
                withXSRFToken ||
                (withXSRFToken !== false &&
                    isURLSameOrigin_default(newConfig.url))
            ) {
                const xsrfValue =
                    xsrfHeaderName &&
                    xsrfCookieName &&
                ...
                if (_config.cancelToken || _config.signal) {
                    ...
                    if (_config.signal) {
                        _config.signal.aborted
                            ? onCanceled()
                            : _config.signal.addEventListener(
                                  'abort',
                                  onCanceled
                              );
                    }
    ...
    var isResolvedHandle = (adapter) =>
        utils_default.isFunction(adapter) ||
        adapter === null ||
        adapter === false;
    ...
    ['object', 'boolean', 'number', 'function', 'string', 'symbol'].forEach(
        (type, i) => {
            validators[type] = function validator(thing) {
                return (
                    typeof thing === type || 'a' + (i < 1 ? 'n ' : ' ') + type
                );
            };
        }
    );
    ...
    function assertOptions(options, schema, allowUnknown) {
        ...
        while (i-- > 0) {
            ...
            if (validator) {
                ...
                const result =
                    value === void 0 || validator(value, opt, options);
                if (result !== true) {
                    throw new AxiosError_default(
    ...
    var Axios = class {
        ...
        _request(configOrUrl, config) {
            ...
            config.headers = AxiosHeaders_default.concat(
                contextHeaders,
                headers
            );
    ...
    var CancelToken = class _CancelToken {
        constructor(executor) {
            ...
            executor(function cancel(message, config, request) {
                ...
                token.reason = new CanceledError_default(
                    message,
                    config,
                    request
                );
    ...
    var displayMap = (locations) => {
        const map2 = L.map('map', {
            ...
            zoomSnap: 0.3,
            // important for perfect fit of the bounds
        });
        ...
        L.polyline(latLngs, { color: 'white' }).addTo(map2);
        map2.fitBounds(bounds.pad(0.6));
    };
